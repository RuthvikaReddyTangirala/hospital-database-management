...
-- Creating DMK
CREATE MASTER KEY
ENCRYPTION BY PASSWORD = 'Group@12';
...
BEGIN
    ...
    ELSE
    BEGIN
    	SET @copay_amount = @billing_amount;
    END
...
RETURN @Count
END;
ALTER TABLE Prescription ADD CONSTRAINT MedicationLimit 
CHECK (dbo.PrescriptionCount(record_id) < 6);
...
RETURN @Count
END;
ALTER TABLE Appointment ADD CONSTRAINT AppointmentLimit 
CHECK (dbo.AppointmentCount(patient_id) < 4);
...
SELECT staff_id, account_number, account_type
